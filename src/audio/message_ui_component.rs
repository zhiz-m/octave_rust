use std::{sync::{atomic::{AtomicBool, Ordering}, Arc}, time::Duration, collections::{HashMap, hash_map::Entry}};

use futures::StreamExt;
use serenity::{model::{prelude::{Message, ChannelId, component::{ButtonStyle, InputTextStyle, ActionRowComponent}, interaction::{message_component::MessageComponentInteraction, InteractionResponseType, modal::ModalSubmitInteraction}, UserId}}, prelude::Context, builder::{CreateActionRow, CreateButton, CreateInputText, CreateEmbed, CreateSelectMenu, CreateSelectMenuOption}};
use songbird::tracks::TrackCommand;
use tokio::{time::timeout, sync::Mutex};
...
impl MessageUiComponent{
    ...
    fn initiate_tasks(&mut self) {
        ...
        
        {
            ...
            tokio::spawn(async move {
                ...
                let mut mci_iter = m.await_component_interactions(&context.shard).timeout(Duration::from_secs(3600)).build();
                loop {
                    while let Ok(Some(mci)) = timeout(Duration::from_millis(MESSAGE_UI_COMPONENT_CHAIN_INTERVAL_MS), mci_iter.next()).await{
                        if let Err(why) = Self::process_message_interaction(&mci, &context, &user_state_map, &audio_state).await.map_err(|e| e.to_string()){
        ...
        
        {
            ...
            tokio::spawn(async move {
                // we will stop waiting for interactions after 1 hour
                let mut mci_iter = m.await_modal_interactions(&context.shard).timeout(Duration::from_secs(3600)).build();
                loop {
                    while let Ok(Some(mci)) = timeout(Duration::from_millis(MESSAGE_UI_COMPONENT_CHAIN_INTERVAL_MS), mci_iter.next()).await{
                        if let Err(why) = Self::process_modal_interaction(&mci, &context, &user_state_map, &audio_state).await.map_err(|e| e.to_string()){
                            println!("error in interaction_loop: {}", why);
                        }
                    }
                    if should_cleanup.load(Ordering::Relaxed){
    ...
    async fn process_message_interaction(mci: &Arc<MessageComponentInteraction>, context: &Arc<Context>, user_state_map: &Arc<Mutex<HashMap<UserId, UserState>>>, audio_state: &Arc<AudioState>) -> Result<(), String>{
        ...
        match id {
            ...
            "shuffle_selection" => {
                ...
                match selections.iter().nth(0).expect("no selections").as_str(){
            ...
            "queue" => {
                ...
                response = mci.create_interaction_response(context, |r| {
                    ...
                    .interaction_response_data(move |d| {
                        d.add_embed(get_styled_embed(&mut CreateEmbed::default(), &text).to_owned())
                    })
                }).await;
    ...
    async fn process_modal_interaction(mci: &Arc<ModalSubmitInteraction>, context: &Arc<Context>, user_state_map: &Arc<Mutex<HashMap<UserId, UserState>>>, audio_state: &Arc<AudioState>) -> Result<(), String> {
        let id = mci.data.custom_id.as_str();
        if id != "song_query_modal" {
            return Err("process_modal_interaction: id was not \"song_query_modal\"".to_owned());
        }
        let component = mci.data.components.iter().map(|x| x.components.iter()).flatten().nth(0).ok_or("process_modal_interaction: no ActionRowComponent")?;
